...
function add_flows_by_client() {
    for src_ip in {100..149}; do
        ...
        ovs-ofctl add-flow a5 "table=0,priority=10,tcp,in_port=${in_port},nw_src=${nw_src},nw_dst=${nw_vip},tp_dst=${nw_vport} action=mod_dl_dst=${dl_dst},mod_nw_dst=${nw_dst},mod_tp_dst=${dst_port},output=${out_port}" -O OpenFlow13
